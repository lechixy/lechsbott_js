    ...
    async execute(client, message, args, cmd, Discord){
        ...
        if(cmd === 'ban'){
            ...
            if(!message.member.permissions.has("BAN_MEMBERS")){
                ...
                .setAuthor(`You are not able to use this command`, user.displayAvatarURL({dynamic: true}))
                .addField(`Need to`, `Ban Members`)
                return message.channel.send({ embeds: [permembed] })
            }

            if(!message.guild.me.permissions.has('BAN_MEMBERS')){
                const embed = new Discord.MessageEmbed()
                .setDescription(`**Lechsbott needs to** \`Ban Members\` **permission to execute this command!**`)
            ...
            .setDescription(`<@${member.id}> banned from server`)
