    ...
    async execute(client, message, args, cmd, Discord) {
        ...
        if (server_queue.songs.length < 2) {
            ...
            let queue1 = new Discord.MessageEmbed()
                ...
                .setDescription(`<@${server_queue.songs[0].addedid}> added from **${server_queue.songs[0].app}**\n
                ${elapsedtime}<:transparent:875400047045525545>${splitbar(bar)}<:transparent:875400047045525545>${totaltime}`)
                .addField(`In Queue`, `${string}`)

                if (queue1.fields[0].value.length >= 1024)
                queue1.fields[0].value = `${queue1.fields[0].value.substr(0, 1024)}...`;
                return message.channel.send({ embeds: [queue1] })
            ...
            let queue1 = new Discord.MessageEmbed()
                ...
                ${elapsedtime}<:transparent:875400047045525545>${splitbar(bar)}<:transparent:875400047045525545>${totaltime}`)
                .addField(`In Queue`, `${string}`)
                .addField('\u200b', `more **__${server_queue.songs.length-11}__** songs from queue`)
                message.channel.send({ embeds: [queue1] })
        }
