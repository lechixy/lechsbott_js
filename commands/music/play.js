        ...
        if (cmd === 'play' || cmd === 'p'){
            ...
            if(args[0].includes(spotifyurl)){
                ...
                for(const result of Object.values(data)){
                    ...
                    const search_title = `${result.artist} ${result.title}`
                    ...
                    if(spotifyplaylist) {
                        song = {
                            ...
                            urlcustom: spotify_finder.link,
                            time: spotifyplaylist.duration.timestamp,
                            title: search_title
                        }
...
async function handleVideo(video, message, args, voice_channel, playlist = false){
...
const queue_song = (message, server_queue, args, client) => {
    ...
    if(!server_queue) {
        let server_queueembed =  new Discord.MessageEmbed()
        .setAuthor(name= `There is any **lechsbott song queue** for **${message.guild.name}**...`, icon_url= `${message.author.displayAvatarURL()}`)
...
const leavechannel = async (message, server_queue, client) => {
    ...
    if(!server_queue){
        await voiceChannel.leave()
        message.channel.send(`**📫 Succesfully leaved from \`${message.member.voice.channel.name}\`**`)
    } else {
        server_queue.songs = [];
        server_queue.connection.dispatcher.end();
        message.channel.send(`**📫 Succesfully leaved from \`${message.member.voice.channel.name}\`**`)
...
const remove_song = (message, args, client, server_queue) => {
    ...
    if(isNaN(args[0])) {
        ...
        let deletedsong = args[0]
        server_queue.songs[deletedsong].delete()
...
const skipto = (message, args, server_queue, client) => {
    ...
    if(isNaN(args[0])) {
        ...
        server_queue.connection.dispatcher.end();
        const skippedsong = server_queue.songs[args[0]]
        let infoembed = new Discord.MessageEmbed()
...
const search_cmd = async (message, args, server_queue, client) => {
    ...
    if(!args.length) {
        ...
        .addField(`Youtube`, `<keywords for search>`)
        return message.channel.send(argsembed)
    }
