...
const queue = require('../../models/playerSchema');
...
async function handleResource(
    ...
    const Discord = require('discord.js');
    const profileData = await queue.findOne({ Guild: message.guild.id })
    ...
    if (!profileData) {
        let profile = await queue.create({
        ...
        await queue.findOneAndUpdate({ Guild: message.guild.id }, { $push: { songs: song } })

        const lechsbottPlayer = async (guild, song) => {
            const song_queue = await queue.findOne({ Guild: message.guild.id }) 
            ...
            const chan = client.channels.cache.get(song_queue.text_channel)
            chan.send({ embeds: [playing] })
        ...
        try {
            ...
            console.log(player)
            await queue.findOneAndUpdate({ Guild: message.guild.id }, { $set: { player: player } })

            const req = await queue.findOne({ Guild: message.guild.id }) 
            const connection = Voice.getVoiceConnection(
                req.text_channel,
                'default'
            );
            await queue.findOneAndUpdate({ Guild: message.guild.id }, { $set: { connection: connection } })

            lechsbottPlayer(message.guild, req.songs[0]);
        } catch (err) {
            await queue.deleteOne({ Guild: message.guild.id })
            const embed = new Discord.MessageEmbed().setDescription(
        ...
        queue.findOneAndUpdate({ Guild: message.guild.id }, { $push: { songs: song } });
