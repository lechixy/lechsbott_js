...
module.exports = {
    ...
    async execute(client, message, args, cmd, Discord){
        ...
        if (cmd === 'play' || cmd === 'p'){
            ...
            if(args[0].includes(scurl)){
                ...
                for(const video of Object.values(videos)){
                    const song = await youtube.getVideoByID(video.id)
                    song.type = 'normal'
                    song.app = 'YouTube',
                    song.customurl = args[0],
                    song.addedby = message.author.username,
                    song.addedid = message.author.id,
        ...
        else if(cmd === 'nowplaying' || cmd === 'np') np_song(message, server_queue, client);
...
const np_song = (message, server_queue, client) => {
    ...
    .setTitle(`${playingaudio} ${server_queue.songs[0].title}`)
    .setURL(server_queue.songs[0].customurl)
    .addField(`Added by`, `<@${server_queue.songs[0].addedid}>`, true)
